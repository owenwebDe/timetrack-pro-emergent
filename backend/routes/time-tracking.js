...
router.get('/entries', authMiddleware, async (req, res) => {
  try {
    const { page = 1, limit = 10, project_id, task_id, start_date, end_date } = req.query;
...
router.post('/manual', authMiddleware, [
  ...
  try {
    ...
    const { project_id, task_id, start_time, end_time, description } = req.body;
...
router.get('/reports/team', authMiddleware, requireManager, async (req, res) => {
  try {
