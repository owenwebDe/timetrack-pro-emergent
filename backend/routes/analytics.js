...
router.get('/dashboard', authMiddleware, async (req, res) => {
  try {
    ...
    const avgActivity = entries.length > 0 ? 
      entries.reduce((sum, entry) => sum + entry.activity_level, 0) / entries.length : 0;
    ...
    res.json({
      ...
      productivity_trend: Object.values(dailyTrend).sort((a, b) => new Date(a.date) - new Date(b.date)),
...
router.get('/team', authMiddleware, requireManager, async (req, res) => {
  try {
    ...
    Object.values(teamStats).forEach(user => {
      user.avg_activity = user.total_entries > 0 ? user.avg_activity / user.total_entries : 0;
    ...
    Object.values(dailyProductivity).forEach(day => {
      const entriesCount = entries.filter(e => 
        e.start_time.toISOString().split('T')[0] === day.date
      ).length;
    ...
    // Summary statistics
    const summary = {
      total_hours: entries.reduce((sum, entry) => sum + entry.duration, 0) / 3600,
      total_entries: entries.length,
      active_users: Object.keys(teamStats).length,
      active_projects: Object.keys(projectStats).length,
      avg_activity: entries.length > 0 ? 
        entries.reduce((sum, entry) => sum + entry.activity_level, 0) / entries.length : 0
    };
    
    res.json({
      team_stats: Object.values(teamStats),
      daily_productivity: Object.values(dailyProductivity).sort((a, b) => new Date(a.date) - new Date(b.date)),
...
router.get('/productivity', authMiddleware, async (req, res) => {
  try {
    ...
    Object.values(hourlyData).forEach(hour => {
      hour.avg_activity = hour.entries_count > 0 ? hour.avg_activity / hour.entries_count : 0;
    });
    ...
    const targetHours = period === 'day' ? 8 : period === 'week' ? 40 : period === 'month' ? 160 : 1920;
    ...
    res.json({
      productivity_chart: Object.values(hourlyData).sort((a, b) => a.hour - b.hour),
...
router.post('/reports/custom', authMiddleware, requireManager, async (req, res) => {
  try {
    const { 
      start_date, 
      end_date, 
    ...
    if (metrics.includes('hours')) {
      report.metrics.total_hours = entries.reduce((sum, entry) => sum + entry.duration, 0) / 3600;
      report.metrics.avg_daily_hours = report.metrics.total_hours / Math.ceil((endDate - startDate) / (1000 * 60 * 60 * 24));
    }
    
    if (metrics.includes('activity')) {
      report.metrics.avg_activity = entries.length > 0 ? 
        entries.reduce((sum, entry) => sum + entry.activity_level, 0) / entries.length : 0;
    }
    
    if (metrics.includes('projects')) {
      const projectsWorked = new Set(entries.map(entry => entry.project_id));
      ...
      entries.forEach(entry => {
        ...
        report.metrics.project_distribution[entry.project_id] += entry.duration / 3600;
      });
    }
